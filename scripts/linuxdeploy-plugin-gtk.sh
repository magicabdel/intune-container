...
# Patched copy of linuxdeploy-plugin-gtk.sh for intune-container.
#
# Why this fork exists: the upstream plugin (via linuxdeploy) over-bundles
# host-managed graphics/Wayland libraries — libepoxy and libwayland-* — into the
# AppDir. Those libraries must come from the *host*, because they're tied to the
# user's GPU driver and compositor. When bundled, they shadow the user's newer
# copies and break EGL on modern/hybrid GPUs (libepoxy aborts with
# "Could not create default EGL display: EGL_BAD_PARAMETER"), giving a blank
# window. See tauri-apps/tauri#9394.
#
# The only change from upstream is the cleanup block at the very end. Drop this
# file into ~/.cache/tauri/linuxdeploy-plugin-gtk.sh before `tauri build` so it
# takes precedence over the version Tauri downloads.
...
# --- intune-container patch ------------------------------------------------
# Remove host-managed graphics/Wayland libraries that linuxdeploy bundles as
# transitive dependencies of WebKitGTK. These MUST come from the host (they are
# bound to the GPU driver and the running compositor); bundling them shadows the
# user's copies and breaks EGL on modern/hybrid GPUs ("Could not create default
# EGL display: EGL_BAD_PARAMETER" from libepoxy → blank window). See
# tauri-apps/tauri#9394.
echo "Dropping host-managed graphics/Wayland libs from the AppDir (intune-container)"
find "$APPDIR/usr/lib" -maxdepth 3 \
    \( -name 'libepoxy.so*' -o -name 'libwayland-*.so*' \) \
